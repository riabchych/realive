...
let schema = require(path.join(global.config.paths.schemas_dir, '/validation/review-validation-schema'));
            ...
            switch (req.params.action) {
                case 'new':
                    ...
                    if (!_.has(params, 'review') && !_.has(params, 'review.to')) {
                        ...
                        req.body.from = req.isAuthenticated() && _.has(req, 'user._doc._id') ? req.user.id : undefined;
                        req.body.isPrivate = params.review.isPrivate || false;
                        req.body.isHidden = req.isAuthenticated() ? _.isEqual(params.review.isHidden, 'on') : true;
                ...
                case 'list':
                    if (_.has(req.body, 'page') && _.has(req.body, 'uid')) {
                        ...
                        return ReviewModel
                            .readReviews(req.body.uid, skips)
                            .then(result => {
                ...
                case 'remove':
                    if (req.isAuthenticated() && _.has(req.body, 'id')) {
                        return ReviewModel.delete(req.user.id, req.body.id)
                            .then(data => {
