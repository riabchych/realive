...
/**
 * CLaut library
 * Copyright(c) 2012 Evgeny Riabchych <riabchych@gmail.com>
 * MIT Licensed
 */
...
DOM = {
    ...
    createElement: function (tagName, attr, style) {
        ...
        if (attr) Base.extend(el, attr);
        if (style) CSS.setStyle(el, style);
        return el;
    },
    ...
    createFragment: (function (doc) {
        ...
        return range && range.createContextualFragment ? function (html) {
    ...
    clone: function (obj) {
        return Base.isArray(obj) ? obj.slice() : Base.extend({}, obj);
    },
    show: function (elem) {
        if (arguments.length > 1) {
            for (var i = 0, l = arguments.length; i < l; ++i) {
                this.show(arguments[i]);
            }
            return;
        }
    ...
    hide: function (elem) {
        ...
        if (l > 1) {
            for (var i = 0; i < l; i++) {
                this.hide(arguments[i]);
            }
            return;
        }
        if (!elem || !elem.style) return;
        var d = CSS.getStyle(elem, 'display');
        elem.olddisplay = (d != 'none') ? d : '';
        elem.style.display = 'none';
    },
    isVisible: function (elem) {
        if (!elem || !elem.style) return false;
        return CSS.getStyle(elem, 'display') != 'none';
    },
    toggle: function (elem, v) {
        if (v === undefined) {
            v = !this.isVisible(elem);
        }
        if (v) {
            this.show(elem);
        }
    ...
    getByClass: function (searchClass, node, tag) {
        ...
        var els = this.getByTag(tag, node);
    ...
    getByTag: function (searchTag, node) {
        return (node || document).getElementsByTagName(searchTag);
    },
    getByTagOne: function (searchTag, node) {
        node = node || document;
        return node.querySelector && node.querySelector(searchTag) || this.getByTag(searchTag, node)[0];
    },
    ...
    getByAttrVal: function (searchTag, attr, value, node) {
        var el = this.getByTag(searchTag, node),
            tagElements = new Array();
        Base.each(el, function (o, i) {
    ...
    replace: function (object, content) {
        if (el && el.parentNode) {
...
CSS = {
    ...
    addClass: function (obj, name) {
        if (obj && !CSS.hasClass(obj, name)) {
            obj.className = (obj.className ? obj.className + ' ' : '') + name;
        }
    },
    removeClass: function (obj, name) {
        if (obj) {
            obj.className = Base.trim((obj.className || '').replace((new RegExp('(\\s|^)' + name + '(\\s|$)')), ' '));
        }
    },
    toggleClass: function (obj, name, v) {
        if (v === undefined) {
            v = !this.hasClass(obj, name);
        }
        (v ? this.addClass : this.removeClass)(obj, name);
    },
    ...
    getStyle: function (elem, name, force) {
        if (Base.isArray(name)) {
            var res = {};
            Base.each(name, function (i, v) {
                res[v] = CSS.getStyle(elem, v);
            });
            return res;
        }
        if (force === undefined) {
    ...
    setStyle: function (elem, name, value) {
        ...
        if (typeof name == 'object') {
            return Base.each(name, function (k, v) {
                CSS.c(elem, k, v);
            });
        }
        if (name == 'opacity') {
    ...
    getSize: function (elem, withoutBounds) {
        var s = [0, 0],
        ...
        if (elem == document) {
            ...
            function getWH() {
                ...
                Base.each(s, function (i, v) {
                    var which = i ? ['Top', 'Bottom'] : ['Left', 'Right'];
                    Base.each(which, function () {
                        s[i] -= parseFloat(CSS.getStyle(elem, 'padding' + this)) || 0;
                        s[i] -= parseFloat(CSS.getStyle(elem, 'border' + this + 'Width')) || 0;
                    });
                });
...
Ajax = {
    ...
    ajaxToQuery: function (qa) {
        ...
        Base.each(qa, function (val, key) {

            if (val != null || !Base.isFunction(val)) {
                if (Base.isArray(val)) {
                    for (var i = 0, c = 0; i < val.length; ++i) {
                        if (val[i] == null || isFunction(val[i])) {
                            continue;
                        }
                        query.push(enc(key) + '[' + c + ']=' + enc(val[i]));
                        ++c;
                    }
                }
                else {
                    query.push(enc(key) + '=' + enc(val));
                }
            }
        });
        query.sort();
    ...
    serializeForm: function (form) {
        ...
        Base.each(g('input'), function (e, i) {
            if ((e.type != 'radio' && e.type != 'checkbox') || e.checked) return nv(i, e);
        });
        Base.each(g('select'), nv);
        Base.each(g('textarea'), nv);
    ...
    setRequestHeaders: function () {
        ...
        var headers = {
            ...
            'Accept': 'text/javascript, text/html, application/json, text/javascript, text/plain, application/xml, text/xml, */*'
        };

        if (self.options.method == 'POST') {
            headers['Content-type'] = 'application/x-www-form-urlencoded' + (this.options.encoding ? '; charset=' + self.options.encoding : '');

            if (self.transport.overrideMimeType && (navigator.userAgent.match(/Gecko\/(\d{4})/) || [0, 2005])[1] < 2005) {
                headers['Connection'] = 'close';
            }
        }

        if (typeof self.options.requestHeaders == 'object') {
            var extras = self.options.requestHeaders;

            if (Base.isFunction(extras.push)) for (var i = 0, length = extras.length; i < length; i += 2)
            headers[extras[i]] = extras[i + 1];
            else Base.each(extras, function (v, k) {
                headers[k] = v;
            });
        }

        Base.each(headers, function (v, k) {
            self.transport.setRequestHeader(k, v);
        });
    },
    ...
    register: function (responderToAdd) {
        ...
        if (responderToAdd == this.responders[i]) return;
    ...
    dispatchException: function (exception) {
        ...
        this.dispatch('onException', this, exception);
    },
    respondToReadyState: function (readyState) {
        ...
        var res = null;
        if (event == 'Complete') {
            res = this.transport.responseText;
            var contentType = this.header('Content-type') || '';
            if (contentType.match(/^\s*(text|application)\/(x-)?(javascript|ecmascript|json)(;.*)?\s*$/)) {
                res = this.evalResponse();
            }

            try {
                var responseSuccess = this.responseIsSuccess() ? 'Success' : 'Failure';
                /*@formatter:off*/ (this.options['on' + this.transport.status] || this.options['on' + responseSuccess] || this.emptyFunction)(res);
                /*@formatter:on*/
            }
            catch (e) {}

            try {
                (this.options['on' + event] || this.emptyFunction)(transport, json);
                this.dispatch('on' + event, this, json);
            }
            catch (e) {}
    ...
    evalResponse: function () {
        try {
            return Base.parseJSON(this.transport.responseText);
        }
        catch (e) {
    ...
    send: function (options) {
        this.options = Base.extend({
            ...
            onSuccess: this.emptyFunction(),
            onError: this.emptyFunction()
        }, options || {});

        try {
            var params = this.options.params ? '&' + this.options.params : '';
            var cache = (!this.options.cache) ? 't=' + Base.now() + params : params;
            this.options.url += (this.options.url.indexOf('?') > 0 ? '&' + cache : '?' + cache);
            ...
            this.start = function () {
                ...
                var body = this.options.postBody ? (this.options.postBody || params) : null;
                this.transport.send(body);
            };
            if (this.options.sync && this.transport.overrideMimeType) {
                this.onStateChange();
            }
...
Base = {
    ...
    parseJSON: (window.JSON && JSON.parse) ? JSON.parse : function (obj) {
        return eval('(' + obj + ')');
    },
    ...
    extend: function () {
        ...
        if (typeof target !== 'object' && !Base.isFunction(target)) target = {};
    ...
    irand: function (mi, ma) {
        return Math.floor(this.rand(mi, ma));
    },
    ...
    match: function (element, selector) {
        if (Base.isString(selector)) return Prototype.Selector.match(element, selector);
    ...
    url2obj: function (hash) {
        var action;
        if (hash.substr(0, 3) == '#!/') {
            action = hash.substr(2);
        }
        else {
            action = hash;
        }
        ...
        if (this.indexOf(action, '?') > 0) {
            ...
            obj.params = {};
            this.each(properties, function (k, v) {
                var p = k.split(/=/);
                obj.params[p[0]] = p[1];
            });
        }
...
Uploader = {
    options: {
        ...
        data: {},
        chooseButton: {},
        onChange: Base.emptyFunction,
        onDragEnter: Base.emptyFunction,
        onDragLeave: Base.emptyFunction,
        onDragOver: Base.emptyFunction,
        onDrop: Base.emptyFunction,
        onProgress: Base.emptyFunction,
        onLoad: Base.emptyFunction
    },
    over: {},
    leave: {},
    init: function (options) {
        ...
        this.options.dropBox = DOM.getById(this.options.dropBox);
        this.options.imageFile = DOM.getById(this.options.imageFile);
        this.options.chooseButton = DOM.getById(this.options.chooseButton);
    ...
    getBuilder: function (file, filedata, boundary) {
        ...
        Base.each(this.options.data, function (i, val) {
    ...
    send: function (e) {
        xhr = new XMLHttpRequest(), start_time = new Date().getTime(), boundary = '----imageboundary' + (new Date).getTime(), xhr.upload.file = Uploader.options.imageFile;
    ...
    return {
        show: function (o) {
            ...
            if (!d1) {
                ...
                d2.className = 'dialog-title-wrap';
                d21 = document.createElement('div');
                d21.className = 'dialog-close-button';
                d22 = document.createElement('div');
                d22.className = 'dialog-title';
                d3 = document.createElement('div');
                d3.className = 'dialog-body';
                d4 = document.createElement('div');
                d4.className = 'dialog-controls';
                d41 = document.createElement('button');
                d41.className = 'blue-button f-right';
                ...
                d22.innerHTML = v.boxTitle;
                d41.innerHTML = 'Р—Р°РєСЂС‹С‚СЊ';

                m.onclick = d21.onclick = d41.onclick = TINY.box.hide;
                window.onresize = TINY.box.resize
            }
            else {
                CSS.setStyle(d1, 'display', 'none');
                clearTimeout(d1.ah);
                if (d21.v) {
                    d2.removeChild(d21);
                    d21.v = 0
            ...
            d1.id = v.boxid;
            m.id = v.maskid;
            CSS.setStyle(d1, 'position', (v.fixed ? 'fixed' : 'absolute'));
            if (v.html && !v.animate) {
                d3.innerHTML = v.html;
                CSS.setStyle(d3, 'width', (v.width ? v.width + 'px' : 'auto'));
                CSS.setStyle(d3, 'height', (v.height ? v.height + 'px' : 'auto'))
            }
            else {
                if (!v.animate && v.width && v.height) {
                    CSS.setStyle(d3, 'width', v.width + 'px');
                    CSS.setStyle(d3, 'height', v.height + 'px');
                }
                else {
                    CSS.setStyle(d3, 'width', '100px');
                    CSS.setStyle(d3, 'height', '100px');
                }
            }
            if (v.mask) {
                this.mask();
            ...
            if (v.autohide) {
                d1.ah = setTimeout(TINY.box.hide, 1000 * v.autohide)
            }
            else {
        ...
        psh: function (c, a, w, h) {
            ...
            var x = CSS.getStyle(d3, 'width'),
                y = CSS.getStyle(d3, 'height');
            if (!w || !h) {
                CSS.setStyle(d3, 'width', (w ? w + 'px' : ''));
                CSS.setStyle(d3, 'height', (h ? h + 'px' : ''));
                CSS.setStyle(d1, 'display', '');
                if (!h) {
                    h = parseInt(d3.offsetHeight)
                }
                if (!w) {
                    w = parseInt(d3.offsetWidth)
                }
                CSS.setStyle(d1, 'display', 'none');
            }
            CSS.setStyle(d3, 'width', x);
            CSS.setStyle(d3, 'height', y);
            this.size(w, h, a)
        },
        esc: function (e) {
            e = e || window.event;
        ...
        mask: function () {
            CSS.setStyle(m, 'height', (this.total(1) + 'px'));
            CSS.setStyle(m, 'width', (this.total(0) + 'px'))
        },
        pos: function () {
            ...
            if (typeof v.top != 'undefined') {
                ...
                t = (this.height() / v.topsplit) - (d1.offsetHeight / 2);
                t = t < 20 ? 20 : t
            }
            if (!v.fixed && !v.top) {
                t += this.top()
            ...
            CSS.setStyle(d1, 'top', (t + 'px'));
            CSS.setStyle(d1, 'left', (typeof v.left != 'undefined' ? v.left + 'px' : (this.width() / 2) - (d1.offsetWidth / 2) + 'px'));
        },

        alpha: function (e, d, a) {
            clearInterval(e.ai);
            if (d) {
                CSS.setStyle(e, 'opacity', 0);
                CSS.setStyle(e, 'display', 'block');
                TINY.box.pos()
            }
        ...
        ta: function (e, a, d) {
            var o = Math.round(CSS.getStyle(e, 'opacity') * 100);
            if (o == a) {
                clearInterval(e.ai);
                if (d == -1) {
                    CSS.setStyle(e, 'display', 'none');
                    e == d1 ? TINY.box.alpha(m, - 1, 0, 2) : d3.innerHTML = '';
                }
                else {
                    if (e == m) {
                        this.alpha(d1, 1, 100)
        ...
        size: function (w, h, a) {
            if (a) {
                ...
                var wp = parseInt(CSS.getStyle(d3, 'paddingLeft').split('px')[0]) + parseInt(CSS.getStyle(d3, 'paddingRight').split('px')[0]);

                var hp = parseInt(CSS.getStyle(d3, 'paddingTop').split('px')[0]) + parseInt(CSS.getStyle(d3, 'paddingBottom').split('px')[0]);

                w += wp;
                h += hp;

                var wd = parseInt(CSS.getStyle(d3, 'width')) > w ? -1 : 1,
                    hd = parseInt(CSS.getStyle(d3, 'height')) > h ? -1 : 1;

                /*@formatter:on*/

                w -= wp;
                h -= hp;
                d3.si = setInterval(function () {
                    TINY.box.ts(w, wd, h, hd, wp, hp)
                }, 20)
            }
            else {
                if (v.close) {
                ...
                CSS.setStyle(d3, 'width', (w + 'px'));
                CSS.setStyle(d3, 'height', (h + 'px'));
                CSS.setStyle(d3, 'display', '');
                this.pos();
                if (v.openjs) {
                    v.openjs()
                }
        ...
        ts: function (w, wd, h, hd, wp, hp) {

            /*@formatter:off */

            var cw = parseInt(CSS.getStyle(d3, 'width')) - wp,
                ch = parseInt(CSS.getStyle(d3, 'height')) - hp;

            /*@formatter:on*/

            if (cw == w && ch == h) {
                clearInterval(d3.si);
                CSS.setStyle(d1, 'display', 'block');
                if (v.close) {
                    d2.appendChild(d21);
                    d21.v = 1
                }
                ...
                if (cw != w) {
                    CSS.setStyle(d3, 'width', (w - Math.floor(Math.abs(w - cw) * .6) * wd) + 'px');
                    console.log('' + cw + ' , ' + w);
                }
                if (ch != h) {
                    CSS.setStyle(d3, 'height', (h - Math.floor(Math.abs(h - ch) * .6) * hd) + 'px');
                    console.log('' + ch + ' , ' + h);
                }
                this.pos()
            }
...
function timeAgo(time) {
    time = Math.round((Date.now() - (time * 1000)) / 1000);
...
//  jeikaps
//
//  Created by Yevhenii Riabchych on 2012-09-12.
...
opinion.toggleForm = function (e) {
    if (!opinion.showedForm) {
        $('#show-form-link').css('display', 'none');
        $('#opinion-form-container').fadeIn(200);
        opinion.showedForm = true;
    }
    else {
        $('#show-form-link').fadeIn();
        $('#opinion-form-container').css('display', 'none');

        opinion.showedForm = false;
    }

};
opinion.load = function (e) {
    var self = $(e);
    self.html('<div class="loading"></div>');
    $.ajax({
        type: "POST",
        url: "/opinions/load",
        data: "_csrf=" + encodeURIComponent(jeikaps.csrfToken) + "&uid=" + encodeURIComponent(jeikaps.uid) + "&offset=" + opinion.countAll,
        dataType: "json",
        error: function () {
            console.log('error');
        ...
        success: function (a) {
            var isMore = a.length > 9 ? true : false;
            var html = new EJS({
                url: '/templates/opinions.ejs'
            });
            var opinions = html.render({
                opinions: a,
                isAuthor: jeikaps.isAuthor,
                uid: jeikaps.uid
            });
            opinion.countAll += a.length;
            var opinions = $(opinions);
            if (opinion.showedOnlyAnonymous) {
                opinions.insertBefore(self);
                $.each(opinions, function (i, o) {
                    o = $(o);
                    if (o.hasClass('opinion-unknown')) {
                        o.css('display', 'none');
                    }
                });
            }
            else {
                $('#wall-title').html('РњРЅРµРЅРёСЏ (' + opinion.countAll + ')');
                opinions.insertBefore(self);
            }
...
opinion.toggleOpinions = function (e) {
    var self = $(e);
    var data = $('.opinion-unknown');
    var title = $('#wall-title');
    if (!opinion.showedOnlyAnonymous) {
        self.html('РїРѕРєР°Р·Р°С‚СЊ РІСЃРµ');
        title.html('РњРЅРµРЅРёСЏ (' + (opinion.countAll - data.length) + ')');
        data.slideUp(150);
        opinion.showedOnlyAnonymous = true;
    }
    else {
        self.html('СЃРєСЂС‹С‚СЊ Р°РЅРѕРЅРёРјРЅС‹Рµ');
        title.html('РњРЅРµРЅРёСЏ (' + opinion.countAll + ')');
        data.slideDown(150);
        opinion.showedOnlyAnonymous = false;
    }
};

opinion.remove = function (timestamp) {
    $.ajax({
        type: "POST",
        url: "/opinions/delete",
        data: "_csrf=" + jeikaps.csrfToken + "&timestamp=" + timestamp,
        dataType: "json",
        cache: false,
        error: function () {
            console.log('error');
        ...
        success: function (a) {
            $('#opinion-' + jeikaps.uid + '-' + timestamp).remove();
            opinion.countAll -= 1;
        }
    });
};
...
profile.follow = function (o) {
    self = $(o);
    $.ajax({
        type: "POST",
        url: "/user/follow",
        data: "_csrf=" + encodeURIComponent(jeikaps.csrfToken) + "&uid=" + jeikaps.uid,
        dataType: "json",
        error: function () {
            console.log('error');
        },
        success: function (a) {
            if (a.status == 1) {
                self.html('РћС‚РїРёСЃР°С‚СЊСЃСЏ');
                self.attr('onclick', 'profile.unfollow(this)');
            }
        }
    });
};

profile.unfollow = function (o) {
    self = $(o);
    $.ajax({
        type: "POST",
        url: "/user/unfollow",
        ...
        success: function (a) {
            if (a.status == 1) {
                self.html('РџРѕРґРїРёСЃР°С‚СЊСЃСЏ');
                self.attr('onclick', 'profile.follow(this)');
            }
        }
    });
};

profile.changeImage = function (c) {
    var caller = c;
    var el = $('#dropbox');
    if (el) {
        caller.show();
        ...
        MessageBox.show(c);
        var uploadProgressHolder = DOM.getById('upload-progress-holder');
        var uploadProgressBar = uploadProgressHolder.firstChild.nextSibling;
        var dropBox = uploadProgressHolder.parentNode;
        Uploader.init({
            dropBox: 'dropbox',
            ...
            onLoad: function (response) {
                ...
                var uploadProgress, previewContainer, imageHolder, uploadedImage, uploadedBadge;
                uploadProgress = DOM.getById('upload-container');
                previewContainer = DOM.createElement('div', {
                    className: 'preview'
                });
                imageHolder = DOM.createElement('span', {
                    className: 'imageHolder'
                });
                uploadedBadge = DOM.createElement('span', {
                    className: 'uploaded'
                });
                image = DOM.createElement('img', {
                    src: '/images/userpics/' + response.text
                });
                ...
                DOM.getById('current-profile-image').src = '/images/userpics/thumbs/' + response.text;
                var c = uploadProgressHolder.parentNode.parentNode;
                c.innerHTML = '';
                c.appendChild(previewContainer);
                DOM.show(uploadedBadge);
...
function updateTime() {
    var qa = $('.timeago');
    $.each(qa, function (i, o) {
        o = $(o);
        o.html(timeAgo(parseInt(o.attr('datetime') || o.attr('title'))))
    });
}
...
function timeAgo(time) {
    ...
    if (time < 60) return 'С‚РѕР»СЊРєРѕ С‡С‚Рѕ';
    else if (time < 3600) return declOfNum((time - (time % 60)) / 60, ['РјРёРЅ', 'РјРёРЅ', 'РјРёРЅ']);
    else if (time < 86400) return declOfNum((time - (time % 3600)) / 3600, ['С‡', 'С‡', 'С‡']);
    else if (time < 2073600) return declOfNum((time - (time % 86400)) / 86400, ['Рґ', 'Рґ', 'Рґ']);
    else if (time < 62208000) return declOfNum((time - (time % 2073600)) / 2073600, ['РјРµСЃ', 'РјРµСЃ', 'РјРµСЃ']);
}
...
$(document).ready(function () {
    ...
    var opinions = $('#profile-opinions article');
    opinion.countAll = opinions.length;

    opinions.live('mouseover', function (e) {
        var qa = $(this).children().children('.opinion-actions');
        qa.css('opacity', 1);
    });

    opinions.live('mouseout', function (e) {
        var qa = $(this).children().children('.opinion-actions');
        qa.css('opacity', 0);
    });

    updateTime();
    var utime = setInterval(updateTime, 60000);
    ...
    $("#opinion-text").live("keyup", function () {
        var o = $(this);
        if (o.val().length > 400) {
            o.val(o.val().slice(0, 400));
        }
        $("#opinion-word-counter").html(400 - o.val().length)
    });
