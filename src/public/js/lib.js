MessageBox = Class.create({
  ...
  initialize: function (options) {
    ...
    if (patern.test(this.options.body)) {
      this.options.body = this.options.body.replace(patern,
        '<img$1 data-preloader="1" />')
  ...
  refreshBox: function () {
    ...
    this.boxContainer.setStyle({
      'width': typeof (this.options.height) === 'string'
        ? this.options.height
  ...
  boxRefreshCoords: function (cont) {
    ...
    this.boxContainer.style.left = ((b_dims.width / 2) - (e_dims.width / 2)) +
      'px'
