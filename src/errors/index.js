...
errors = {
    ...
    renderErrorPage: function (code, err, req, res, next) {
        /*jshint unused:false*/
        var self = this,
            defaultErrorTemplatePath = require(path.join(global.config.paths.views_dir, '/error'));
        ...
        function renderErrorInt(errorView) {
            ...
            res.status(code).render((errorView || 'error'), {
                ...
                return res.status(500).send(
                    ...
                    '<pre>' + _.escape(templateErr.message || templateErr) + '</pre>' +
                    '<br ><p>whilst trying to render an error page for the error: </p>' +
                    code + ' ' + '<pre>' + _.escape(err.message || err) + '</pre>'
                );
            });
        }
    ...
    error404: function (req, res, next) {
        var message = res.isAdmin && req.user ? 'No User Found' : 'Page Not Found';
